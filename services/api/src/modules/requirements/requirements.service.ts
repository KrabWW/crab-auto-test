...
const STATUSES = new Set<RequirementStatus>(["draft", "reviewed", "approved"]);
...
export class RequirementsService {
  ...
  async submitReview(projectId: string, requirementId: string, actorId: string): Promise<RequirementDto> {
    ...
    return this.transition(projectId, requirementId, actorId, "submit-review", "reviewed");
  }
  ...
  async approve(projectId: string, requirementId: string, actorId: string): Promise<RequirementDto> {
    ...
    const existing = await this.findRequirement(projectId, requirementId);
    if (existing.status !== "reviewed") throw new BadRequestException("Only reviewed requirements can be approved");
  ...
  private async transition(
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      await tx.requirement.update({
        ...
        data: {
          status: toStatus,
          ...(toStatus === "reviewed" ? { reviewedBy: actorId } : {}),
      ...
      await tx.requirementVersion.update({
        ...
        data: {
          status: toStatus,
          ...(toStatus === "reviewed" ? { reviewedAt: now } : {}),
